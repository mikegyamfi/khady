...
def collection_products(request, category_name):
    if models.Category.objects.filter(name=category_name, status=True).exists():
...
def product_details(request, category_name, prod_name):
    if models.Category.objects.filter(name=category_name, status=True):
        if models.Product.objects.filter(name=prod_name, status=True):
            product = models.Product.objects.get(name=prod_name, status=True)
            product_images = models.ProductImage.objects.filter(product=product)
            main_image = product_images.first()
            category = models.Category.objects.get(name=category_name, status=True)
            context = {
                'product': product,
...
def checkout(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ref = 'GHBAY' + str(random.randint(11111111, 99999999))
